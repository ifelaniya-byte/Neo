    ...
    def _add_pool_update_events(self, block: int) -> None:
        ...
        for pool_id, reserves in self.pool_reserves.items():
            event = NormalizedEvent(
                ...
                entity_id=pool_id,
                payload={"pool_id": pool_id, "reserve0": reserves[0], "reserve1": reserves[1]},
                raw_payload_hash=sha256_hash({"pool_id": pool_id, "reserves": reserves})
            )
            self.events.append(event)
...
class TimeCausalFeatureStore:
        ...
        for event in price_events:
            ...
            if token_id and price is not None:
                if token_id not in latest_prices or event.event_timestamp_ms > latest_prices[token_id][0]:
                    latest_prices[token_id] = (event.event_timestamp_ms, price, event.event_id)

        for token_id, (timestamp, price, event_id) in latest_prices.items():
            age_ms = decision_time_ms - timestamp
            features[f"price_{token_id}"] = FeatureValue(
                name=f"price_{token_id}",
                value=price,
                source_event_ids=[event_id],
                max_available_timestamp_ms=timestamp,
    ...
    def _build_liquidity_features(self, events: List[NormalizedEvent], decision_time_ms: int) -> Dict[str, FeatureValue]:
        ...
        for event in pool_events:
            ...
            reserves = event.payload.get("reserves", [0, 0])
            if reserves:
                total_liquidity += sum(reserves)

        if pool_events:
            latest_event = max(pool_events, key=lambda e: e.event_timestamp_ms)
            age_ms = decision_time_ms - latest_event.event_timestamp_ms

            features["total_liquidity"] = FeatureValue(
                name="total_liquidity",
                value=total_liquidity,
                source_event_ids=[e.event_id for e in pool_events],
                max_available_timestamp_ms=latest_event.event_timestamp_ms,
        ...
        for event in self.events:
            if event.block_number <= block:
                if event.event_type == EventType.PRICE:
                    ...
                    pool_id = event.payload.get("pool_id")
                    reserves = event.payload.get("reserves")
                    if pool_id and reserves:
                        state["reserves"][pool_id] = reserves
...
class OutcomeEngine:
    ...
    @staticmethod
    def settle(action: ActionCandidate, quote_result: Dict, execution_result: Dict,
               costs: Dict, state_after: Dict) -> OutcomeLabel:
        """Settle the final outcome."""
...
class LabelBuilder:
    ...
    def _build_label_for_candidate(self, packet: DecisionPacket, candidate: ActionCandidate,
        ...
        # Settle outcome
        outcome = self.outcome_engine.settle(candidate, quote_result, execution_result, costs, state_after)
...
class ConstraintEngine:
    ...
    def check_constraints(self, packet: DecisionPacket, candidate: ActionCandidate,
        ...
        # Check freshness
        max_age = max(packet.provenance.get("feature_source_timestamps_ms", {}).values()) if packet.provenance.get("feature_source_timestamps_ms") else 0
        decision_time = packet.as_of.get("decision_timestamp_ms", 0)
        age_ms = decision_time - max_age

        constraints.append({
            "name": "freshness",
            "passed": age_ms <= self.config.max_feature_age_ms,
            "value": age_ms,
            "limit": self.config.max_feature_age_ms,
            "reason_code": ReasonCode.STALE_DATA if age_ms > self.config.max_feature_age_ms else None
        })
        ...
        constraints.append({
            "name": "gas_limit",
            "passed": prediction.aleatoric_std <= self.config.constraints.max_gas_usd,
            "value": prediction.aleatoric_std,
...
class ConservativePlanner:
    ...
    def calculate_lcb(self, prediction: ModelForecast, conformal_quantile: float) -> float:
        ...
        lcb = prediction.pnl_q05 - conformal_quantile
...
class AbstentionGate:
    ...
    def decide(self, packet: DecisionPacket, candidate: ActionCandidate,
        ...
        # Check loss limit
        if prediction.pnl_q05 < -self.config.constraints.max_loss_usd:
...
class PaperBroker:
    ...
    def execute_decision(self, decision: DecisionOutput) -> Optional[OutcomeLabel]:
        ...
        # In paper mode, we would simulate the outcome
        # For now, return a placeholder
        outcome = OutcomeLabel(
            decision_id=decision.decision_id,
            action_id=decision.action_id or "",
            quoted_output_usd=0.0,
            realized_output_usd=0.0,
            input_cost_usd=0.0,
            gas_usd=decision.estimated_gas_usd or 0.0,
            protocol_fees_usd=0.0,
            borrow_fees_usd=0.0,
            bridge_fees_usd=0.0,
            slippage_cost_usd=decision.estimated_slippage_usd or 0.0,
            revert_cost_usd=0.0,
            other_costs_usd=0.0,
            gross_pnl_usd=0.0,
            net_pnl_usd=0.0,
    ...
    def backtest(self, policy: BaselinePolicy, packets: List[DecisionPacket],
        ...
        for packet in packets:
            ...
            if decision.verdict == Verdict.TRADE:
                ...
                label = next((l for l in labels if l.decision_id == packet.decision_id), None)
                if label:
...
class StressTestEngine:
    ...
    def run_stress_tests(self, policy: BaselinePolicy, packets: List[DecisionPacket],
        ...
        # S2: Liquidity reduction
        stress_results["liquidity_10pct"] = self._liquidity_reduction_test(policy, packets, labels, 0.1)
        stress_results["liquidity_30pct"] = self._liquidity_reduction_test(policy, packets, labels, 0.3)
        stress_results["liquidity_50pct"] = self._liquidity_reduction_test(policy, packets, labels, 0.5)

        # S3: Quote latency
        stress_results["latency_1s"] = self._quote_latency_test(policy, packets, labels, 1000)
        stress_results["latency_10s"] = self._quote_latency_test(policy, packets, labels, 10000)
    ...
    def _liquidity_reduction_test(self, policy: BaselinePolicy, packets: List[DecisionPacket],
                                 labels: List[OutcomeLabel], reduction_factor: float) -> Dict[str, Any]:
        """Test with reduced liquidity."""
        # This would modify packet features to simulate lower liquidity
        # For now, return placeholder
        backtester = Backtester(self.store)
        return backtester.backtest(policy, packets, labels)

    def _quote_latency_test(self, policy: BaselinePolicy, packets: List[DecisionPacket],
                           labels: List[OutcomeLabel], latency_ms: int) -> Dict[str, Any]:
        """Test with quote latency."""
        # This would modify packet features to simulate stale quotes
        # For now, return placeholder
        backtester = Backtester(self.store)
        return backtester.backtest(policy, packets, labels)
...
@cli.command()
def all(mode: str = "synth", preset: str = "research", seed: int = 42, config: str = None):
    ...
    if TORCH_AVAILABLE:
        console.print("[yellow]Training placeholder - full implementation would train ensemble[/yellow]")
    else:
    ...
    console.print("[yellow]Calibration placeholder - full implementation would calibrate model[/yellow]")
    manifest.update_stage("calibrate")

    console.print("[bold blue]Stage 8: Paper replay[/bold blue]")
    console.print("[yellow]Paper replay placeholder - full implementation would replay decisions[/yellow]")
    manifest.update_stage("replay")

    console.print("[bold blue]Stage 9: Backtest[/bold blue]")
    console.print("[yellow]Backtesting placeholder - full implementation would run backtests[/yellow]")
    manifest.update_stage("backtest")

    console.print("[bold blue]Stage 10: Stress test[/bold blue]")
    console.print("[yellow]Stress testing placeholder - full implementation would run stress tests[/yellow]")
