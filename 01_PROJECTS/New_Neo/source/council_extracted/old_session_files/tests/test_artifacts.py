...
class ArtifactTests(unittest.TestCase):
    def test_qwen_weight_pin(self):
        ...
        # this sandbox has no transformers — must not claim runnable
        if not rec["transformers_available"]:
            self.assertFalse(rec["runnable_offline"])
            self.assertEqual(rec["status"], "files-complete-load-unproven")
