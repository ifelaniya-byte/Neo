...
def verify_offline_artifact(model_dir=None, expected_weight_hash=None):
    ...
    rec = {
        ...
        "tokenizer_offline_safe": None,
        "chat_template_embedded": None,
        "tokenizer_remote_refs": None,
    ...
    for fn in sorted(os.listdir(root)):
        ...
        pin = PINNED.get(fn)
        # Override pin for model.safetensors if expected_weight_hash provided
        if fn == "model.safetensors" and expected_weight_hash:
            pin = {"sha256": expected_weight_hash, "bytes": None}
        row = {"bytes": size, "sha256": digest, "pinned": bool(pin)}
        if pin:
            row["pin_match"] = digest == pin["sha256"] and (pin["bytes"] is None or size == pin["bytes"])
...
if __name__ == "__main__":
    ...
    expected_weight_hash = sys.argv[2] if len(sys.argv) > 2 else None
    rec = verify_offline_artifact(model_dir, expected_weight_hash)
